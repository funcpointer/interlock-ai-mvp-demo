...
def run_eval(run_dir: Path, eval_path: Path) -> tuple[bool, list[str]]:
    ...
    for expected in spec.get("expected_coverage_warnings", []):
        if not _matches_any(coverage_findings, expected):
...
def _matches_any(findings: list[dict[str, Any]], matcher: dict[str, Any]) -> bool:
    return any(_matches(finding, matcher) for finding in findings)
