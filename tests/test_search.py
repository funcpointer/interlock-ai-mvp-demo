...
from interlock_mvp.core.search import build_search_records, expand_query, search_records_in_memory, search_run, write_search_index
...
def test_in_memory_search_uses_same_review_records_without_persistence() -> None:
    evidence = [
        EvidenceItem(
            evidence_id="ev1",
            doc_id="A",
    ...
    hits = search_records_in_memory(records, "XFMR rating")

    assert hits
    assert hits[0]["record_id"] == "ev1"
    assert hits[0]["retrieval_methods"] == ["memory"]
